...
let sumNumbers = (string) => {
  let sum = string.reduce(function(total, value){
    return total + value;
  })
  console.log(sum)
}
...
console.log(computerReminder(10, 2))
console.log(computerReminder(4,0))
/*-----------------------------------------------------------------
Challenge: 06-range
Difficulty: basic
Prompt:
...
let range = (int1 , int2) => {
  if(int1 > int2){
    console.log(int1 + "must be less than" + int2)
  }
  let array = []
  array.push(int1);
...
let reverseUpcaseString = (string) => {
 let array = []
 let somthing = string.toUpperCase();
 return somthing.split("").reverse().join("");
}
...
let formatWithPadding = (x, string, z) => {
  let newArray = []
  let number = x.toString();
  let newNum1 =  z - number.length;
  if(number.length >= number[z]){
    console.log(x);
  }else{
    let newNum = string.repeat(newNum1);
...
return string == str.toLowerCase().split('').reverse().join('');
};
...
let mumble = (string) => {
  let mumbleArray = []
    ...
    let newNum = string[i].repeat(i+1);
    mumbleArray.push(newNum);
  }
  console.log(mumbleArray.join('-'))
}
mumble('abc'); //=> 'a-bb-ccc'
mumble('121'); //=> '1-22-111'
mumble('!A 2'); //=> '!-AA-   -2222'
...
let mergeObjects = (...arguments) => {
    let newObj = {};
        let mergerFunc = (mergeObj) => {
        for (let prop in mergeObj) {
          if (mergeObj.hasOwnProperty(prop)) {
            newObj[prop] = mergeObj[prop];
          }
        }
      };
    for (let i=0; i < arguments.length; i++) {
      mergerFunc(arguments[i]);
    }
  return newObj;
}
...
let findHighestPriced = (array) =>{
  let highestPrice = 0;
  let object;
  array.forEach(function(skuPrice){
    if (skuPrice.price > highestPrice){
      highestPrice = skuPrice.price;
      object = skuPrice;
    }
  });
  return object;
...
let reduceArray = (array, callBack, accInitial) => {
      var accumulator = accInitial;
      array.forEach(function(element, index) {
...
let flatten = (array) => {
  ...
  array.forEach(function(x) {
    if (Array.isArray(x)) {
      flatted = flatted.concat(flatten(x));
    } else {
      flatted.push(x);
    }
  });
...
let intersection = (x, y) =>{
  ...
  var y2 = [...y];
  x.forEach(value => {
    var index = y2.indexOf(value);
    if (index > -1) {
      array.push(y2.splice(index, 1)[0]); // at the index position, remove that item; push to y2.
    }
  }); 
  return array;
}
...
let countTheBits = (num) => {
  console.log(num.toString(2).split('').filter(bit => bit == '1').length)
}
...
function addChecker(numbers, result) {
  var addTotal = false;
  var start = 0
  var end = numbers.length - 1; 
  while (start < end) {
    var total = numbers[start] + numbers[end];
    if (total === result) return true;
    total < result ? start++ : end--;
  }
  return addTotal;
}
